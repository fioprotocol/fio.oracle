...
process.argv[2]
  ...
  .forEach((jobName) => {
    if (jobName === REGISTERED_JOBS_LIST.wrapEth) {
      const errItems = fs
        .readFileSync(LOG_FILES_PATH_NAMES.wrapEthTransactionErrorQueue)
        .toString();
    ...
    if (jobName === REGISTERED_JOBS_LIST.unwrapEth) {
      const errItems = fs
        .readFileSync(LOG_FILES_PATH_NAMES.unwrapEthTransactionErrorQueue)
        .toString();
    ...
    if (jobName === REGISTERED_JOBS_LIST.wrapPoly) {
      const errItems = fs
        .readFileSync(LOG_FILES_PATH_NAMES.wrapPolygonTransactionErrorQueue)
        .toString();
    ...
    if (jobName === REGISTERED_JOBS_LIST.unwrapPoly) {
      const errItems = fs
        .readFileSync(LOG_FILES_PATH_NAMES.unwrapPolygonTransactionErrorQueue)
        .toString();
...
console.log(
  'Successfully Completed. It is necessary to wait some time for Jobs to process new transactions.',
);

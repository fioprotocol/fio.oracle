      ...
      if (!lastProcessedBlockNumber) {
        ...
        if (fetchLastBlockNumber) {
          ...
          lastBlockNumberInChain =
            (await fetchLastBlockNumber()) - blocksOffset;
        }
        createLogFile({
          filePath,
...
export const addLogMessage = ({
  ...
  if (isJsonFormat) {
    if (addTimestamp)
      message[
        timestampTitle && timestampTitle.length ? timestampTitle : 'timeStamp'
      ] = timeStamp;
...
export const updateBlockNumberForTokensUnwrappingOnETH = (blockNumber) => {
  fs.writeFileSync(
    LOG_FILES_PATH_NAMES.blockNumberUnwrapTokensETH,
    blockNumber
  );
};
...
export const updateBlockNumberForDomainsUnwrappingOnETH = (blockNumber) => {
  fs.writeFileSync(
    LOG_FILES_PATH_NAMES.blockNumberUnwrapDomainETH,
    blockNumber
  );
};
...
export const updateBlockNumberMATIC = (blockNumber) => {
  fs.writeFileSync(
    LOG_FILES_PATH_NAMES.blockNumberUnwrapDomainPolygon,
    blockNumber
  );
};
...
export const updatePolygonNonce = (nonce) => {
  fs.writeFileSync(
    LOG_FILES_PATH_NAMES.polygonNonce,
    nonce ? nonce.toString() : ''
  );
};
...
export const getLastProceededBlockNumberOnFioChain = () => {
  return parseFloat(
    fs.readFileSync(LOG_FILES_PATH_NAMES.blockNumberFIO, 'utf8')
  );
};
...
export const handleLogFailedBurnNFTItem = ({
  logPrefix,
  burnData,
  errorLogFilePath,
}) => {
